    ...
    <div class="{{ request()->routeIs('clients.show') ? 'col-md-8' : 'col-md-12' }}">
        <div class="form-group">
            ...
            <input type="text" class="form-control {{ $errors->first('name') ? 'is-invalid' : '' }}" name="name"
                id="name" placeholder="Nombre completo" value="{{ old('name', $client->name) }}">
    ...
    <div class="col-md-4">
        <div class="form-group">
            ...
            <input type="text" class="form-control {{ $errors->first('rfc') ? 'is-invalid' : '' }}" name="rfc"
                id="rfc" placeholder="RFC" value="{{ old('rfc', $client->rfc) }}">
    ...
    <div class="col-md-4">
        <div class="form-group">
            ...
            <input type="email" class="form-control {{ $errors->first('email') ? 'is-invalid' : '' }}" name="email"
                id="email" placeholder="Ingresar email" value="{{ old('email', $client->email) }}">
                ...
                <optgroup label="Medidor actual">
                    @if (isset($actual_measurer))
                        <option value="{{ $actual_measurer->id }}">
                            {{ $actual_measurer->brand }}, {{ $actual_measurer->model }},
                            {{ $actual_measurer->serial_number }}
                        </option>
                    @else
